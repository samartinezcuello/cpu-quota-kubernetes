...
def store_data_in_db(records):
    ...
    for record in records:
        ...
        # No se tienen en cuenta PODs del namespace "kube-system"
        if record['namespace'] == 'kube-system' or record['namespace'] == 'default':
            continue
        ...
        cur.execute("""
        INSERT IGNORE INTO namespaces_cpu (namespace, cpu_time_total, cpu_quota)
        VALUES (%s, %s, %s)
        """, (record['namespace'], 0, 100))
    ...
    for db_pod in db_pods:
        ...
        if db_pod_id not in existing_pods:
            # Pod no encontrado en la lista de métricas actuales por lo que el pod ya no existe, se actualiza namespace
            cur.execute("""
            INSERT INTO namespaces_cpu (namespace, cpu_time_total)
            VALUES (%s, %s)
            ON DUPLICATE KEY UPDATE
                cpu_time_total = cpu_time_total + VALUES(cpu_time_total)
            """, (db_namespace, db_cpu_usage))
            
            # Eliminar registro de la tabla cpu_metrics
